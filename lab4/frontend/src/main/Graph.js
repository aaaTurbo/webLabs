...
function Figure({r}) {
    ...
    return (<>
        <Expression id={'g1'} latex={`0<=y<=${r} \\{-${r}/2<=x<=0\\}`} color={color} lines={false}/>
        <Expression id={'g3'} latex={`x^2+y^2<=${r}^2/4 \\{x>=0\\} \\{y<=0\\}`} color={color} lines={false}/>
        <Expression id={'g2'} latex={`${-r} < x + y \\{-${r}<=x<=0\\} \\{-${r}<=y<=0\\}`} color={color} lines={false}/>
